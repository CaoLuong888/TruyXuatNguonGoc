...
const Key = () => {
    ...
    const handleDelete = async (values) => {
        try {
            const response = await axios.put(`http://localhost:3000/api/luukey/${user.matk}`, {
                tenchuky: null,
                publickey: null,
            });
    ...
    const checkPasswd = async (values) => {
        try {
            const response = await axios.post(`http://localhost:3000/api/checkpasswd/${user.matk}`, {
    ...
    const handletaokey = async (matk) => {
        try {
            const response = await axios.post(`http://localhost:3000/api/taoKey/${matk}`);
